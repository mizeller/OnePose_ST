...
def in_box(keypoints, box):
    ...
    for i, (m1_, m2_) in enumerate(zip(m1, m2)):
        if (
            m1_ > 0
            and m2_ > 0
            and m1_ < np.dot(vec_01, vec_01)
            and m2_ < np.dot(vec_02, vec_02)
        ):
            # ret_idxs.append(i)
            in_box_valid[i] = 1
...
def run_triangulation(
    colmap_path, model_path, database_path, image_dir, empty_model, verbose=True
):
...
def main(
    ...
    stats = run_triangulation(
        colmap_path, model, database, image_dir, empty_sfm_model, verbose=verbose
    )
    os.system(
        f"colmap model_converter --input_path {model} --output_path {outputs_dir}/model.ply --output_type PLY"
    )
