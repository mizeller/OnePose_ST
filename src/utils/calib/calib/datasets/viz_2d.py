...
import math
import cv2
from src.utils.calib.calib.datasets.view import read_image, resize_image
from loguru import logger


def plot_images(
    imgs, titles=None, cmaps="gray", dpi=100, pad=0.5, adaptive=True, autoscale=True
):
...
def add_text(idx, text, pos=(0.01, 0.99), fs=15, color="w", lcolor="k", lwidth=2):
    ax = plt.gcf().axes[idx]
    t = ax.text(
        *pos,
        text,
...
def plot_row(dict_list_main, pred_annotate=["roll", "rho", "fov"], titles=[], **kwargs):
    ...
    for j in range(len(dict_list)):
        ...
        if "roll" in pred_annotate or "rho" in pred_annotate:
            pred_angle = (
                dict_list[j]["roll"] * np.pi / 180 if "roll" in pred_annotate else 0
            )
            distorted_offset = (
                dict_list[j]["rho"] * im.shape[0] if "rho" in pred_annotate else 0
            )
            radius = 5000
            ...
            cv2.line(im, (pred_x2, pred_y2), (pred_x1, pred_y1), (255, 0, 0), 10)

        if "roll" in pred_annotate:
            texts1.append(
                {
                    "idx": j,
                    "text": f"roll (pred): {pred_angle*180/np.pi:.3f}°",
                    "pos": (0.01, 0.99),
                    "fs": 15,
                    "color": "#00ff00",
                    "lcolor": "k",
        ...
        if "rho" in pred_annotate:
            texts2.append(
                {
                    "idx": j,
                    "text": f"rho (pred): {dict_list[j]['rho']:.3f} ratio",
                    "pos": (0.01, 0.92),
                    "fs": 15,
                    "color": "#00ff00",
                    "lcolor": "k",
        ...
        if "fov" in pred_annotate:
            texts3.append(
                {
                    "idx": j,
                    "text": f"fov (pred): {dict_list[j]['vertical_fov']:.3f}°",
                    "pos": (0.01, 0.85),
                    "fs": 15,
                    "color": "#00ff00",
                    "lcolor": "k",
        ...
        if "k1_hat" in pred_annotate and "k1_hat" in dict_list[0]:
            texts4.append(
                {
                    "idx": j,
                    "text": f"k1_hat (pred): {(dict_list[j]['k1_hat']):.3f}",
                    "pos": (0.01, 0.78),
                    "fs": 15,
                    "color": "#00ff00",
                    "lcolor": "k",
