...
import numpy as np
import torch
from src.utils.calib.calib.datasets.view import (
    read_image,
    numpy_image_to_torch,
    resize_image,
)
from src.utils.calib.calib.datasets.viz_2d import plot_row
from src.utils.calib.calib.utils.experiments import load_experiment
...
class DeepCalibration:
    def __init__(self, device=None):
        checkpoint_path = Path(torch.hub.get_dir(), "deepcalib", "checkpoint_best.tar")
    ...
    def calibrate(self, image, force_pinhole=True):
        ...
        num_bins = 256
        roll_centers = torch.linspace(
            -45.0, 45.0 + (90.0 / (num_bins - 1)), num_bins + 1
        )
        rho_centers = torch.linspace(-1.0, 1.0 + (2.0 / (num_bins - 1)), num_bins + 1)
        fov_centers = torch.linspace(
            20.0, 105.0 + (85.0 / (num_bins - 1)), num_bins + 1
        )

        roll = roll_centers[pred["roll"].argmax(1).item()]
        rho = (rho_centers[pred["rho"].argmax(1).item()]) * 0.35
        vfov = fov_centers[pred["vfov"].argmax(1).item()]
        fy_px = h / 2 / torch.tan(vfov * (torch.pi / 180.0) / 2)
        ...
        if not force_pinhole:
            k1_hat_centers = torch.linspace(
                -0.45, 0.0 + (0.45 / (num_bins - 1)), num_bins + 1
            )
